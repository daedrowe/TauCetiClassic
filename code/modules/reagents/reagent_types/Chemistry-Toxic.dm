...
/datum/reagent/toxin/hemosynthase
	name = "Hemosynthase"
	id = "hemosynthase"
	description = "An extremely aggressive haematological mutagen that forcefully rewrites blood type antigens. The process is excruciatingly painful and causes severe genetic damage."
	reagent_state = LIQUID
	color = "#8b0000"
	custom_metabolism = 0.4
	toxpwr = 0
	taste_message = "liquid agony"

	data = list()

/datum/reagent/toxin/hemosynthase/on_general_digest(mob/living/carbon/human/H)
	. = ..()
	if(!.)
		return
	...
	// Constant pain throughout (Highly unstable and damaging)
	H.adjustToxLoss(8 * REM)
	H.adjustCloneLoss(6 * REM)
	H.radiation += 5

	if(prob(70))
		to_chat(H, "<span class='userdanger'>Ваши кости ломит невыносмиой болью!</span>")
		H.emote("scream")
	if(prob(50))
		H.Weaken(5)
		H.make_jittery(40)
	if(prob(35))
		H.vomit()
	if(prob(20))
		to_chat(H, "<span class='userdanger'>Вы чувствуете, как ваша кровь буквально закипает!</span>")
		H.adjustFireLoss(10)

	// Scramble a random SE block (genetic instability)
	if(prob(20) && length(H.dna.SE))
		var/block = rand(1, length(H.dna.SE))
		H.dna.SetSEValue(block, rand(1, 4095))
		domutcheck(H, null)

	// At tick 10, change blood type
	if(data["ticks"] >= 10)
		...
		blood_types -= H.dna.b_type
		H.dna.b_type = pick(blood_types)
		H.fixblood(FALSE)

		// Final burst of damage
		H.adjustCloneLoss(15)
		H.adjustToxLoss(10)
		H.Weaken(10)

		// Fracture a random bone for extra pain
		var/obj/item/organ/external/BP = pick(H.bodyparts)
		if(BP && !(BP.status & ORGAN_BROKEN))
			BP.fracture()

		holder.del_reagent("hemosynthase")
